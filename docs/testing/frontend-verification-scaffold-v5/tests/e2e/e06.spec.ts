...
// E06 - 19 cases. Generated from IS212_PROJECT_TEST_CASES.xlsx.
// Each test.fixme() is a specification. Remove .fixme once implemented.

test.describe('E06-S01 - Search for suitable venues', () => {
   ...
   */
  test.fixme('TC_E06S01_01 - Verify that running a search where multiple criteria match an available venue should list it wi', async ({ page }) => {
  ...
  /**
   * TC_E06S01_02
   * AC:      Scenario 1 - Matching venues listed
   * Sprint:  2.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S01_02 - Verify that when no venue matches every criterion, near matches should be returned instead of a', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as an Event Coordinator
  ...
  /**
   * TC_E06S01_03
   * AC:      Scenario 3 - Undersized venue marked unsuitable
   * Sprint:  2.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S01_03 - Verify that a venue available but with capacity below the event\'s expected attendance should ', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as an Event Coordinator
   ...
   */
  test.fixme('TC_E06S01_04 - Verify that a venue that is blocked or already confirmed for the requested period should not ap', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as an Event Coordinator
...
test.describe('E06-S02 - Check venue suitability against event requirements', () => {

  /**
   * TC_E06S02_01
   * AC:      Scenario 1 - Suitability shown per event
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S02_01 - Verify that viewing a venue in the context of an event with recorded requirements should show ', async ({ page }) => {
    // Steps from the specification:
  ...
  /**
   * TC_E06S02_02
   * AC:      Scenario 2 - Failing requirements named
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S02_02 - Verify that a venue failing one or more recorded requirements should be marked unsuitable with ', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as coordinator_1@connectsphere.com
  ...
  /**
   * TC_E06S02_03
   * AC:      Scenario 3 - All requirements met
   * Sprint:  3.0
   *
   * Pre-conditions:
  ...
  /**
   * TC_E06S02_04
   * AC:      Scenario 4 - Unsuitable venue may still be requested
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S02_04 - Verify that submitting a booking request for a venue marked unsuitable should still be accepted', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as coordinator_1@connectsphere.com
...
test.describe('E06-S03 - Request a venue booking', () => {

  /**
   * TC_E06S03_01
   * AC:      Scenario 1 - Request created as Pending
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S03_01 - Verify that submitting a booking request for an event with no existing pending request should ', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as coordinator_1@connectsphere.com
  ...
  /**
   * TC_E06S03_02
   * AC:      Scenario 2 - First request moves event to Planning
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S03_02 - Verify that submitting the first booking request for an Approved event should move its status t', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as coordinator_1@connectsphere.com
  ...
  /**
   * TC_E06S03_03
   * AC:      Scenario 3 - Second request for same event blocked
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S03_03 - Verify that submitting a second booking request for an event that already has a pending reques', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as coordinator_1@connectsphere.com
  ...
  /**
   * TC_E06S03_04
   * AC:      Scenario 4 - Venue taken by another event blocked
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S03_04 - Verify that submitting a request for a venue already Pending or Confirmed for another event ove', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as coordinator_1@connectsphere.com
   ...
   */
  test.fixme('TC_E06S03_05 - Verify that an Event Coordinator should be able to view the status of each of their own booking', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as coordinator_1@connectsphere.com
...
test.describe('E06-S04 - Decide on a venue booking request', () => {

  /**
   * TC_E06S04_01
   * AC:      Scenario 1 - Request approved and confirmed
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S04_01 - Verify that approving a pending request for a venue that is free for the period should confirm ', async ({ page }) => {
    // Steps from the specification:
  ...
  /**
   * TC_E06S04_02
   * AC:      Scenario 2 - Rejected with reason and alternative
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S04_02 - Verify that rejecting a request with a recorded reason and a suggested alternative venue should', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as venue_staff_1@connectsphere.com
  ...
  /**
   * TC_E06S04_03
   * AC:      Scenario 3 - Rejection without reason blocked
   * Sprint:  3.0
   *
   * Pre-conditions:
...
test.describe('E06-S06 - Prevent double-booking of a venue', () => {

  /**
   * TC_E06S06_01
   * AC:      Scenario 1 - Overlapping approval blocked
   * Sprint:  3.0
   *
   * Pre-conditions:
   *   "Riverside Hall" has a confirmed booking for "Charity Run" on 15/01/2027, 09:00–17:00
   *   A second pending request for "Riverside Hall" on 15/01/2027, 14:00–18:00 exists for event "Product Expo 2026"
   *
   * Test data:
   ...
   */
  test.fixme('TC_E06S06_01 - Verify that attempting to approve a request that overlaps an existing confirmed booking for the', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as venue_staff_1@connectsphere.com
  ...
  /**
   * TC_E06S06_02
   * AC:      Scenario 2 - Competing request flagged
   * Sprint:  3.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme('TC_E06S06_02 - Verify that approving one of two pending requests for the same venue and overlapping times shou', async ({ page }) => {
    // Steps from the specification:
    // 1. Log in as venue_staff_1@connectsphere.com
  ...
  /**
   * TC_E06S06_03
   * AC:      Scenario 3 - Simultaneous approval fails safely
   * Sprint:  3.0
   *
   * Pre-conditions:
   *   Two pending requests exist for "Riverside Hall" on 25/01/2027, overlapping times
   *   venue_staff_2@connectsphere.com approves one of them a moment before venue_staff_1@connectsphere.com processes the other
   *
   * Test data:
   ...
   */
  test.fixme('TC_E06S06_03 - Verify that if another Venue Staff member approves a conflicting request moments earlier, a sim', async ({ page }) => {
    // Steps from the specification:
