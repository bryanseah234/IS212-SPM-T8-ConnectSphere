...
test.describe("E01-S01", () => {
  ...
  /**
   * TC_E01S01_03
   * AC:      E01-S01 - Scenario 3 (Lockout on fifth failure)
   * Sprint:  1.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme("TC_E01S01_03 - Verify that after 5 consecutive incorrect password attempts, the next sign-in attempt should show the account locked and explain how to reset it, even if that next attempt uses the correct password", async ({ page }) => {
    // Steps from the specification:
  ...
  /**
   * TC_E01S01_04
   * AC:      E01-S01 - Scenario 4 (Reset link issued)
   * Sprint:  1.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme("TC_E01S01_04 - Verify that requesting a password reset on a locked account should send a reset link to the registered email", async ({ page }) => {
    // Steps from the specification:
  ...
  /**
   * TC_E01S01_05
   * AC:      E01-S01 - Scenario 5 (Lock lifted on new password)
   * Sprint:  1.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme("TC_E01S01_05 - Verify that setting a new password via a valid reset link should lift the lock and allow sign-in with the new password", async ({ page }) => {
    // Steps from the specification:
  ...
  /**
   * TC_E01S01_06
   * AC:      E01-S01 - Scenario 3 (Lockout on fifth failure); cross-ref E14-S02
   * Sprint:  1.0
   *
   * Pre-conditions:
   ...
   */
  test.fixme("TC_E01S01_07 - Verify that a sixth sign-in attempt is refused even with the correct password once the account is locked", async ({ page }) => {
    // Steps from the specification:
    // 1. Enter organiser_a@clienta.com in the Email field
